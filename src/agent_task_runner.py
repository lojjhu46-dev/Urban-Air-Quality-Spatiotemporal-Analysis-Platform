...
def run_custom_city_task(store: AgentTaskStore, task_id: str, config: AgentTaskRunConfig) -> None:
    ...
    try:
        ...
        if action == "collect":
            ...
            stored_output_uri = _store_dataset_result(task_id, result)
            _record_step(
                store,
                task_id,
                ...
                phase="SAVED",
                progress=1.0,
                message=t("collection.progress_saved", config.language, path=stored_output_uri),
                result_payload={
                    "output_path": stored_output_uri,
