...
- 如果你希望分析新的城市，可以在 Historical Data Agent 页面中输入自定义国家/地区和城市名，选择年份、污染物和天气字段，由系统规划并采集新的历史数据集，然后立即切换到分析页面继续使用。
...
- 历史数据 Agent
  - 支持输入自定义国家/地区与城市名，并通过 DeepSeek 与 Open-Meteo 地理编码解析目标城市。
...
历史数据 Agent 页面是本系统的数据扩展入口。它不是单纯的聊天界面，而是一个“自定义城市输入 + 后台采集任务 + 执行反馈”的采集工作台，主要能力包括：
...
`自定义国家/地区与城市输入 -> DeepSeek 位置校验 -> 城市候选解析 -> 后台采集任务 -> 采集计划 -> 数据源查询 -> 分块合并 -> 输出数据集 -> 侧边栏切换 -> 分析页面`
...
1. 根据用户输入的国家/地区和城市名调用 DeepSeek 校验位置。
2. 调用 Open-Meteo 地理编码接口解析城市候选、坐标和时区。
3. 根据城市所在区域确定数据源、起始可用日期和采样步长。
4. 创建后台任务并持续写入任务状态、阶段进度和日志。
5. 按固定分块大小切分采集任务，默认块大小为 90 天。
6. 抓取空气质量字段，并按需补充天气字段。
7. 合并分块结果，输出系统可直接加载的数据集。
8. 生成覆盖率摘要、运行提示和告警信息。
